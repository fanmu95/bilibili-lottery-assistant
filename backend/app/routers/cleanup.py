...
def cleanup_preview(db: Session = Depends(get_db)):
    """预览：统计将删除的已开奖未中奖转发（不实际删除）"""
    stat = cleanup_service.cleanup_transfers(db, dry_run=True)
    if stat.get("error"):
        return {"error": stat["error"]}
    return {
        "dry_run": True,
        "checked": stat["checked"],
        "won_keep": stat["won_keep"],
        "no_notice": stat["no_notice"],
        "not_ended": stat["not_ended"],
...
@router.post("/run")
def cleanup_run(db: Session = Depends(get_db)):
    """执行：删除所有已开奖且未中奖的转发动态（不可恢复，请先预览确认）"""
    stat = cleanup_service.cleanup_transfers(db, dry_run=False)
    if stat.get("error"):
        return {"error": stat["error"]}
    if stat["deleted"]:
        add_log(db, "warning", "activity",
                f"动态清理：删除 {stat['deleted']} 条已开奖未中奖的转发动态")
    return {
        "dry_run": False,
        "checked": stat["checked"],
        "won_keep": stat["won_keep"],
